...
class LuasClient(object):
    ...
    def line_status(self, line=models.LuasLine.Green):
        """
    ...
    def next_tram(self, stop, direction=models.LuasDirection.Inbound):
        """
        ...
        output_tram = None
        for tram in stop_details[ATTR_TRAMS]:
            if direction == models.LuasDirection.Inbound \
                    and tram[ATTR_DIRECTION] == ATTR_INBOUND_VAL:
                output_tram = tram
                break
            elif direction == models.LuasDirection.Outbound \
    ...
    def _build_luas_tram_from_map(tram):
        if tram is not None:
            direction = models.LuasDirection.Inbound
            if tram[ATTR_DIRECTION] == ATTR_OUTBOUND_VAL:
                direction = models.LuasDirection.Outbound

            return models.LuasTram(
                tram[ATTR_DUE],
                direction,
